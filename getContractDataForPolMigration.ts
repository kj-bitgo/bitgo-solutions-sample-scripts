...
function getApproveCallData() {
    ...
    const data = ethAbi.methodID('approve', [ 'address', 'uint256' ]).toString('hex') + ethAbi.rawEncode([ 'address', 'uint256' ], [ '0x29e7DF7b6A1B2b07b731457f499E1696c60E2C4e', amount ]).toString('hex')
...
function getMigrateCallData() {
    // Amount here needs to be replaced with the amount of tokens to be migrated
    const data = ethAbi.methodID('migrate', [ 'uint256' ]).toString('hex') + ethAbi.rawEncode([ 'uint256' ], [ amount ]).toString('hex')
